  ...
  const carouselLogos: Logo[] = Object.entries(skillToIcon)
    ...
    .map(([skill, icon], index) => ({
      ...
      img: (props: React.SVGProps<SVGSVGElement>) => {
        const color = ['Next.js', 'ShadCN UI', 'Express.js', 'Markdown', 'GitHub', 'Vercel'].includes(skill) 
          ? "#ffffff" 
          : `#${icon!.hex}`;
        return (
    ...
    <section id="skills" className="border-b border-border bg-[#03060a]">
      <div className="mx-auto max-w-6xl px-6 py-24 md:px-8 md:py-32">
        <div className="flex flex-col gap-4 mb-16 text-center">
          <p className="text-sm font-semibold text-accent uppercase tracking-widest">Tech Stack</p>
          <h2 className="text-4xl md:text-6xl font-bold tracking-tight text-white">Skills & Technologies</h2>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            The modern toolkit I use to build performant and scalable web applications.
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {skills.flatMap(category => category.skills).map((skill, index) => {
            ...
            const color = iconData ? (
              ['Next.js', 'ShadCN UI', 'Express.js', 'Markdown', 'GitHub'].includes(skill) 
              ? "#ffffff" 
              : `#${iconData.hex}`
            ) : "#ffffff";
            ...
            return (
              ...
              >
                ...
                <span className="text-[12px] font-bold tracking-tight uppercase text-[#c9d1d9] font-sans">
                   {skill.toUpperCase()}
                </span>
              </div>
            );
