...
function AddressBar({
  ...
  const variantStyles = {
    chrome:
      "bg-background/20 rounded-full border border-foreground/10 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.03)_inset] backdrop-blur-md hover:bg-background/30 transition-all cursor-pointer group/address",
    safari:
      "bg-background/10 rounded-lg border border-foreground/10 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.03)_inset] backdrop-blur-sm hover:bg-background/20 transition-all cursor-pointer group/address",
  }
      ...
      {items.map((item, index) => (
        <div
          ...
          className={`
            ...
            ${
              item.active
                ? "bg-primary/5 text-primary border border-primary/5 shadow-sm"
                : "text-muted-foreground hover:text-foreground hover:bg-background/20 hover:backdrop-blur-sm"
            }
          `}
        >
          {item.icon && (
...
export function BrowserWindow({
  ...
  const getHeaderStyles = () => {
    ...
    if (variant === "chrome") {
      return `${baseStyles} bg-background/20 overflow-hidden`
    }

    if (variant === "safari") {
      return `${baseStyles} bg-background/20 overflow-hidden border-b border-foreground/10`
    }

    return `${baseStyles} bg-background/30`
  }

  return (
    <div
      className={`
        relative rounded-2xl border shadow-[0px_1px_1px_0px_rgba(0,_0,_0,_0.05),_0px_1px_1px_0px_rgba(255,_252,_240,_0.5)_inset,_0px_0px_0px_1px_hsla(0,_0%,_100%,_0.1)_inset,_0px_0px_1px_0px_rgba(28,_27,_26,_0.5)] dark:shadow-[0px_1px_1px_0px_rgba(0,_0,_0,_0.2),_0px_1px_1px_0px_rgba(0,_0,_0,_0.3)_inset,_0px_0px_0px_1px_hsla(0,_0%,_0%,_0.2)_inset,_0px_0px_1px_0px_rgba(255,_255,_255,_0.1)]
    ...
    >
      ...
      {showSidebar && sidebarPosition === "top" && (
        <div className="border-b border-foreground/10 bg-background/10 backdrop-blur-sm h-16">
          <SidebarContent
            items={sidebarItems}
            variant="navigation"
      ...
      <div className="flex flex-1 h-0">
        ...
        {showSidebar && sidebarPosition === "left" && (
          <div
            className={`border-r border-foreground/10 bg-background/10 backdrop-blur-sm ${sidebarSizes[size]} flex-shrink-0 h-full`}
          >
            <SidebarContent items={sidebarItems} onItemClick={onSidebarItemClick} />
          </div>
        )}
        ...
        {showSidebar && sidebarPosition === "right" && (
          <div
            className={`border-l border-foreground/10 bg-background/10 backdrop-blur-sm ${sidebarSizes[size]} flex-shrink-0 h-full`}
          >
            <SidebarContent items={sidebarItems} onItemClick={onSidebarItemClick} />
          </div>
        )}
      ...
      {showSidebar && sidebarPosition === "bottom" && (
        <div className="border-t border-foreground/10 bg-background/10 backdrop-blur-sm h-16">
          <SidebarContent
            items={sidebarItems}
            variant="navigation"
            onItemClick={onSidebarItemClick}
